        ...
        if (file_exists($ctrlPath))
        {
            ...
            if (!empty($requestedAction))
            {
                ...
                if (method_exists($controller, $requestedAction)) {
                    print $controller->$requestedAction($requestedParams);
                } else {
                    die($requestedAction . " action not found in " . $controllerName);
                }

            } else {
                if (method_exists($controller, 'index')) {
                    print $controller->index($requestedParams);
                }
            }

            if (file_exists(__DIR__.'/Views/'.$requestedController.'_view.php')) {
                require_once __DIR__.'/Views/'.$requestedController.'_view.php';
            } else {
                require_once __DIR__.'/Views/not_found_view.php';
            }
